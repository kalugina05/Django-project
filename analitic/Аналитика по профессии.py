...
# Фильтрация данных по вакансиям для 1С-разработчиков
vacancy_keywords = ['1С-разработчик', '1с разработчик', '1c разработчик', '1с', '1c', '1 c', '1 с']
data = data[data['name'].str.contains('|'.join(vacancy_keywords), case=False, na=False)]
...
salary_trend['average_salary_rub'] = salary_trend.apply(
    lambda row: row['average_salary_rub'] / 3 if row['year'] < 2009 else row['average_salary_rub'], axis=1
)
...
plt.title("Динамика уровня зарплат по годам для 1С-разработчиков")
plt.xlabel("Год")
